...
Below are the most relevant experience records. Choose the 3–6 best matches — ONLY include records that are directly relevant to the query. If the query is about a specific company, only include records from that company. If the query is about a specific skill or project, only include records that directly address it. Do not include tangentially related records to fill space. Synthesize a 2–3 sentence narrative written in FIRST PERSON as Steve speaking directly — use "I", "my", "I've built", "I believe" etc. Sound authentic and specific, not like a recruiter bio. Suggest 3 smart follow-up questions a visitor might ask. Identify 3–5 dominant themes.
...
Respond with ONLY a valid JSON object in this exact shape, no prose, no markdown fences:
{
  ...
  "themes": ["theme1","theme2","theme3"]
    ...
    if (!parsed || !Array.isArray(parsed.record_ids) || !parsed.record_ids.length) {
      ...
      parsed = {
        summary: `I have direct experience that touches on this — see the matched experiences below for the most relevant work.`,
        record_ids: top,
